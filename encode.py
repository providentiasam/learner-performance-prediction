                ...
                for i, (item_id, ts) in enumerate(df_user[:, 1:3]):
                    ...
                    if "sc" in active_features:
                        for skill_id in Q_mat_dict[item_id]:
                            counts = phi(
                                np.array(
                                    counters[user_id, skill_id, "skill"].get_counters(
                                        ts
                                    )
                                )
                            )
                            attempts[
                                i, skill_id * NUM_WINDOWS : (skill_id + 1) * NUM_WINDOWS
                            ] = counts
                    ...
                    if "ic" in active_features:
                        counts = phi(
                            np.array(
                                counters[user_id, item_id, "item"].get_counters(ts)
                            )
                        )
        ...
        if "w" in active_features:
            ...
            if "tw" in active_features:
                ...
                for i, (item_id, ts, correct) in enumerate(df_user[:, 1:4]):
                    ...
                    if "tc" in active_features:
                        counts = phi(
                            np.array(counters[user_id, "correct"].get_counters(ts))
                        )
            ...
            # Counts
                ...
                if "sc" in active_features:
                    tmp = np.vstack((np.zeros(num_skills), skills))[:-1]
                    corrects = np.hstack((np.array(0), df_user[:, 3])).reshape(-1, 1)[
                        :-1
                    ]
                ...
                if "tc" in active_features:
                    wins[:, -1] = phi(
                        np.concatenate((np.zeros(1), np.cumsum(df_user[:, 3])[:-1]))
                    )
