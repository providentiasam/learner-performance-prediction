...
best_steps=(5165 1761 568 5921 2623)

for i in "${indices[@]}"
do
    command="python train_saint.py --dataset=${datasets[i]} --train_batch=${train_batch[i]} --lr=${lr[i]} --warmup_step=${warmup_step[i]} --dropout_rate=${dropout_rate[i]} --layer_count=${layer_count[i]} --head_count=${head_count[i]} --dim_model=${dim_model[i]} --dim_ff=${dim_ff[i]} --seq_len=${seq_len[i]} --num_epochs=10000"
    echo "$command"
    eval "$command"
done