...
class SelfAttention(nn.Module):
    def __init__(
        ...
        assert (
            dim % head_count
        ) == 0, "dimension must be divisible by the number of config.head_count"

        self.head_count = head_count
        self.dim_head = dim // head_count
    ...
    def forward(
        ...
        **kwargs,
        ...
        b, t, e, h, dim_h = *x.shape, self.head_count, self.dim_head
        ...
        merge_head_count = lambda x: reshape_dim(x, -1, (-1, dim_h)).transpose(1, 2)
        q, k, v = map(merge_head_count, (q, k, v))
        ...
        mask = (
            torch.ones(t, t + total_mem_len, **to(x))
            .triu_(diagonal=1 + total_mem_len)
            .bool()
        )
        ...
        cmem_k, cmem_v = map(merge_head_count, (cmem_k, cmem_v))
        ...
        aux_loss = F.mse_loss(
            attn_fn(q, old_mem_k, old_mem_v), attn_fn(q, cmem_k, cmem_v)
        )
...
class CompressiveTransformer(nn.Module):
    def __init__(self, config):
        ...
        assert cmem_len >= (mem_len // config.cmem_ratio), (
            ...
            f"divided by the compression ratio {int(mem_len // config.cmem_ratio)}"
        )
        assert all(
        ...
        self.to_model_dim = (
            nn.Identity()
            if config.emb_dim == config.dim_model
        ...
        self.pos_emb = nn.Parameter(
            torch.zeros(
                config.head_count,
                seq_and_mem_len,
                config.dim_model // config.head_count,
            )
        )

        # self.to_logits = nn.Sequential(
        #     nn.Identity() if config.emb_dim == dim else nn.Linear(dim, config.emb_dim),
        #     nn.Linear(config.emb_dim, num_tokens)
        # )

        wrapper = Residual
        # wrapper= (
        #     partial(GRUGating, config.dim_model, mogrify=config.mogrify_gru)
        #     if config.gru_gated_residual
        #     else Residual
        # )
                ...
                wrapper(
                    PreNorm(
                        ...
                        SelfAttention(
                            ...
                            reconstruction_attn_dropout=config.dropout_rate,
    ...
    def forward(self, x, memories=None, mask=None):
        ...
        assert t <= self.seq_len, (
            f"input contains a sequence length {t} "
            f"that is greater than the designated maximum sequence length {self.seq_len}"
        )
        ...
        return x, Memory(mem=next_mem, compressed_mem=next_cmem), aux_loss
