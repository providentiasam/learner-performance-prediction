...
if __name__ == '__main__':
    ...
    df_acc = df_acc.unstack('dataset').sort_index(axis=0, level=['test_type', 'group_tag'], ascending=[True, True]).dropna(how='all')
    df_acc['count'] = count_col.iloc[:, 0].astype(int)
    df_acc = df_acc.reset_index(drop=False)
    df_acc.columns = ['GroupBy', 'InteractionType', 'TestType', 'Model'] + \
